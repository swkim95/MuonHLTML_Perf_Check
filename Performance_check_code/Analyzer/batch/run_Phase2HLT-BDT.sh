...
export OUTDIR=${SRC}/Outputs_${VER}/BDT
export WORKDIR=/d0/scratch/msoh/Phase2HLT-BDT_${VER}/${TAG}/${JobID}
...
cp ${SRC}/HLTBDTAnalyzer.C ${WORKDIR}
# cp ${SRC}/HLTBDTAnalyzer_C.* ${WORKDIR}
...
# -- run analyzer
# echo root -l -b -q 'HLTBDTAnalyzer.C+("'$VER'", "'$TAG'", '$DATASET', "'$JobID'", '$doDimuon', '$doGenMatchForIso')'

echo 'gROOT->LoadMacro("HLTBDTAnalyzer.C+"); gSystem->Exit(0);' | root -b -l
root -l -b -q 'HLTBDTAnalyzer.C+("'$VER'", "'$TAG'", '$DATASET', "'$JobID'", '$doDimuon', '$doGenMatchForIso')' >tmp.log
