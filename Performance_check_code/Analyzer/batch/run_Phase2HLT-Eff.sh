...
export OUTDIR=${SRC}/Outputs_${VER}/Eff
export WORKDIR=/d0/scratch/msoh/Phase2HLT-Eff_${VER}/${TAG}/${JobID}
...
cp ${SRC}/HLTEffAnalyzer.C ${WORKDIR}
# cp ${SRC}/HLTEffAnalyzer_C.* ${WORKDIR}
...
# -- run analyzer
# echo root -l -b -q 'HLTEffAnalyzer.C+("'$VER'", "'$TAG'", '$DATASET', "'$JobID'", '$doDimuon', '$doGenMatchForIso')'

echo 'gROOT->LoadMacro("HLTEffAnalyzer.C+"); gSystem->Exit(0);' | root -b -l
root -l -b -q 'HLTEffAnalyzer.C+("'$VER'", "'$TAG'", '$DATASET', "'$JobID'", '$doDimuon', '$doGenMatchForIso')' >tmp.log
