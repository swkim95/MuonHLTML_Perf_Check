...
export OUTDIR=${SRC}/Outputs_${VER}/EffFR
export WORKDIR=/d0/scratch/msoh/Phase2HLT-EffFR_${VER}/${TAG}/${JobID}
...
cp ${SRC}/HLTEffFRAnalyzer.C ${WORKDIR}
# cp ${SRC}/HLTEffFRAnalyzer_C.* ${WORKDIR}
...
# -- run analyzer
# echo root -l -b -q 'HLTEffFRAnalyzer.C+("'$VER'", "'$TAG'", '$DATASET', "'$JobID'", '$doDimuon', '$doGenMatchForIso')'

echo 'gROOT->LoadMacro("HLTEffFRAnalyzer.C+"); gSystem->Exit(0);' | root -b -l
root -l -b -q 'HLTEffFRAnalyzer.C+("'$VER'", "'$TAG'", '$DATASET', "'$JobID'", '$doDimuon', '$doGenMatchForIso')' >tmp.log
