...
export OUTDIR=${SRC}/Outputs_${VER}/Res
export WORKDIR=/d0/scratch/msoh/Phase2HLT-Res_${VER}/${TAG}/${JobID}
...
cp ${SRC}/HLTResAnalyzer.C ${WORKDIR}
# cp ${SRC}/HLTResAnalyzer_C.* ${WORKDIR}
...
# -- run analyzer
# echo root -l -b -q 'HLTResAnalyzer.C+("'$VER'", "'$TAG'", '$DATASET', "'$JobID'", '$doDimuon', '$doGenMatchForIso')'

echo 'gROOT->LoadMacro("HLTResAnalyzer.C+"); gSystem->Exit(0);' | root -b -l
root -l -b -q 'HLTResAnalyzer.C+("'$VER'", "'$TAG'", '$DATASET', "'$JobID'", '$doDimuon', '$doGenMatchForIso')' >tmp.log
